...
impl Client {
    ...
    pub async fn from_credentials(
        ...
        Ok(Client {
            ...
            service: DatastoreClient::with_interceptor(channel, Client::interceptor(token_manager)),
        })
    }

    /// Gets an entity from a key.
    pub async fn get(&mut self, key: Key) -> Result<Option<Entity>, Error> {
        let results = self.get_all(&[key]).await?;
        Ok(results.into_iter().next())
    }

    /// Gets multiple entities from multiple keys.
    pub async fn get_all<T, I>(&mut self, keys: I) -> Result<Vec<Entity>, Error>
    where
    ...
    pub async fn put(&mut self, entity: Entity) -> Result<Option<Key>, Error> {
    ...
    pub async fn put_all<I>(&mut self, entities: I) -> Result<Vec<Option<Key>>, Error>
    where
