...
use crate::datastore::{Entity, Error, Filter, Key, KeyID, Order, Query, Value};
...
impl Client {
    ...
    pub async fn get(&self, key: Key) -> Result<Option<Entity>, Error> {
        let results = self.get_all(&[key]).await?;
        Ok(results.into_iter().next())
    }

    /// Gets multiple entities from multiple keys.
    pub async fn get_all<T, I>(&self, keys: I) -> Result<Vec<Entity>, Error>
    where
    ...
    pub async fn put(&self, entity: Entity) -> Result<Option<Key>, Error> {
        let result = self.put_all(Some(entity)).await?;
        Ok(result.into_iter().next().and_then(|value| value))
    }
    ...
    pub async fn put_all<I>(&self, entities: I) -> Result<Vec<Option<Key>>, Error>
    where
        I: IntoIterator<Item = Entity>,
    {
        ...
        let response = service.commit(request).await?;
        let response = response.into_inner();
        let keys = response
            .mutation_results
            .into_iter()
    ...
    pub async fn delete(&self, key: Key) -> Result<(), Error> {
        self.delete_all(&[key]).await
    }

    /// Deletes multiple entities identified by multiple keys.
    pub async fn delete_all<T, I>(&self, keys: I) -> Result<(), Error>
    where
        I: IntoIterator<Item = T>,
        T: Borrow<Key>,
    {
    ...
    pub async fn query(&self, query: Query) -> Result<Vec<Entity>, Error> {
        let mut service = self.service.lock().unwrap();
        ...
        loop {
            ...
            let results = service.run_query(request).await?;
